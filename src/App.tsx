...
function MenuSection({
  ...
  return (
    <div
      ...
      style={{ background: section.background, borderColor: section.border }}
    >
...
export default function App() {
  const [title, setTitle] = useState("Летний магазинчик");
  const [subtitle, setSubtitle] = useState("Домашние напитки и вкусняшки");
  const [sections, setSections] = useState<MenuSectionData[]>(initialSections);
    ...
    <div className="app">
      <style>{`
        * { box-sizing: border-box; }
        ...
        .title-input { padding: 10px 14px; font-size: 42px; font-weight: 900; }
        .subtitle-input { margin-top: 8px; padding: 10px 14px; font-size: 18px; }
        ...
        .section-title-input { padding: 10px 12px; font-size: 24px; font-weight: 900; }
        .items-list { display: grid; gap: 8px; }
        .item-row { display: grid; grid-template-columns: 1fr 82px 34px; gap: 8px; align-items: center; }
        .item-name-input, .item-price-input { padding: 9px 10px; font-size: 16px; }
        .item-price-input { text-align: right; font-weight: 800; }
        .footer {
          margin-top: 20px;
          padding: 16px;
          border-radius: 24px;
      ...
      <main className="container">
        <section className="header">
          <div className="header-row">
            <div>
              <div className="label no-print">✨ Конструктор меню</div>
            ...
            <div className="buttons no-print">
              <button className="primary-button" onClick={addSection}>+ Раздел</button>
